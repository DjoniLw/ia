...
| 08/04/2026 | Issue #152 | 3 padrões adicionados pelo treinador-agent (code review da issue #152 — Redesign Anamnese): (1) 🔁 REINCIDÊNCIA — `_clinicId` em métodos de repositório — BLOQUEANTE de segurança; nova regra de detecção: `grep -n '_clinicId' *.repository.ts` obrigatório no gate de compliance; regex de detecção adicionada ao item em `backend-seguranca.md`; (2) NOVO — `include` sem `select` na entidade principal expõe campos sensíveis (`signToken`, `signatureUrl`, `passwordHash`) na resposta de API; todo método de repositório chamado por route handler deve usar `select` explícito; item adicionado em `backend-seguranca.md`; (3) NOVO — testes de `$transaction` sem cenário de falha não testam atomicidade — obrigatório ter ao menos 1 `it()` onde o último step lança e o erro propaga; item adicionado em `geral-testes.md`. |
| 08/04/2026 | Issue #152 | 2 padrões adicionados pelo treinador-agent (sugestão de treinamento — consistência cross-layer): (1) NOVO — alterar TTL/prazo de negócio no backend exige busca obrigatória por referências do valor na UI antes de fechar o PR; `TTL_HOURS → TTL_DAYS` não refletido no `SendAnamnesisDialog`; item adicionado em `geral-escopo-pr.md` com comandos de detecção e boa prática de comentário de referência cruzada; (2) NOVO — migração de campo de mídia (base64 → URL) exige busca de todos os `src=` e usos do campo anterior na UI; `signature → signatureHash` não atualizado em componente de visualização; item adicionado em `geral-escopo-pr.md` com checklist de migração. |
