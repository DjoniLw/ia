...
export class ManualReceiptsService {
  ...
  async receive(clinicId: string, billingId: string, dto: CreateManualReceiptDto) {
    ...
    return prisma.$transaction(async (tx) => {
        ...
        data: {
          ...
          lines: {
            create: dto.lines.map((line) => ({
              ...
              amount: line.amount,
              walletEntryId: line.walletEntryId ?? null,
            })),
          },
        },
        include: { lines: true },
      ...
      // 3. Debit wallet entries used as payment
      for (const line of dto.lines) {
        if (
          (line.paymentMethod === 'wallet_credit' || line.paymentMethod === 'wallet_voucher') &&
          line.walletEntryId
        ) {
          await wallet.use(clinicId, line.walletEntryId, line.amount, billingId)
        }
      }
